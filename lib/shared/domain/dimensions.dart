...
class Dimension {
  ...
  static final Map<int, List<dynamic>> thresholds = {
    TEMPERATURE: [[18, 24], [ColorConstants.BLUE, ColorConstants.GREEN, ColorConstants.RED]],
    TVOC: [[220, 1430], [ColorConstants.GREEN, ColorConstants.YELLOW, ColorConstants.RED]],
    CO2: [
      [800, 1000, 1400],
      [ColorConstants.GREEN, ColorConstants.YELLOW, ColorConstants.ORANGE, ColorConstants.RED]
    ],
    ADJUSTED_TEMP_CUBE: [
      [18, 24],
      [ColorConstants.BLUE, ColorConstants.GREEN, ColorConstants.RED]
    ],
    UVI: [
      [3, 6, 8, 11],
  ...
  static dynamic getColor(int dimensionId, double val) {
    ...
    for (int i = 0; i < th.length - 1; i++) {
      if (th[i] <= val && val < th[i + 1]) {
        var rgb = colors[i];
        return material.Color.fromARGB(255, rgb[0], rgb[1], rgb[2]);
      }
    }
    return null;
  }
...
class ColorConstants {
  ...
  static const GREEN_LIGHT = [128, 255, 128];
  static const YELLOW = [255, 255, 0];
  static const ORANGE = [255, 170, 0];
  static const RED = [255, 0, 0];
  static const PURPLE = [179, 0, 179];
  static const BROWN_DARK = [128, 0, 32];
}
